...
def generate_table_events_teacher(df: pd.DataFrame):
    ...
    main_df.loc[-1] = [result_str_teacher, f'{count_type_event["внутренний"]} внутренних',
                       f'{count_type_event["муниципальный"]} муниципальных'
...
def generate_table_student_perf(df: pd.DataFrame):
    ...
    for type in lst_type:
        ...
        main_df = pd.concat([main_df, row_itog])
    # Результирующая строка для всего датафрейма
    all_quantity_student = len(result_df)  # количество участий студентов
    result_student = f'участий обучающихся-{all_quantity_student}\n'

    # Считаем преподавателей
    quantity_teacher = len(result_df['ФИО'].unique())
...
def generate_context(dct_value: dict):
    ...
    context['Общая_информация'] = first_sheet_df[
        ['Дисциплина', 'Дата_рождения', 'Дата_ПОО', 'Стаж', 'Педстаж', 'Стаж_в_ПОО',
    ...
    student_perf_df.columns = ['ФИО', 'Дата', 'Форма', 'Уровень', 'Вид', 'Название', 'Тема', 'ФИО_студента', 'Курс',
                               'Профессия', 'Группа',
                               'Результат', 'Номинация']
            ...
            for teacher, dct_data in dct_value.items():
                ...
                for file in os.listdir(template_folder):
                    if file.endswith('.docx') and not file.startswith(
                            '~$'):  # получаем только файлы docx и не временные
